...
const DB_NAME: string = '__fpoRatingDb';
...
export class Sql {
    ...
    createTables() {
        ...
        this.query('select * from tbl_farmers').then( (data) => { 
            if(data.res.rows.length < 1){
                this.query(`insert into tbl_farmers (
                    fm_fname,
                    fm_mname,
                    fm_lname
                ) values(?, ?, ?)`, ['John','Dove','test'] ).then( (data) => { console.log(data); });
                ...
                this.query(`insert into tbl_farmers (
                    ...
                    fm_mname,
                    fm_lname
                ) values(?, ?, ?)`, ['Jane','Dove','test'] ).then( (data) => { console.log(data); });

                this.query(`insert into tbl_farmers (
                    fm_fname,
                    fm_mname,
                    fm_lname
                ) values(?, ?, ?)`, ['Prathamesh','Dove','test'] ).then( (data) => { console.log(data); });
            }
        });
